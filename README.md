...
- **Custom types live at the adapter layer.** `XqliteEcto3.Types.UUID`, `Instant`, `Duration`, `TimestampTZ`, `Array`, `ExactDecimal`. Each is an `Ecto.Type` or `Ecto.ParameterizedType` module — no magic around how SQLite stores them.
...
The adapter validates every configuration value it forwards to a PRAGMA at connect time. An unrecognized value (`journal_mode: :walk`, `foreign_keys: :nonsense`) is a structured connect error — never SQLite's silent fallback to a default. Every such error names the setting it refused as well as the value: `details` is `%{key: :journal_mode, value: :walk}` and the message reads `config journal_mode rejected: :walk`. The same error reaches the `[:xqlite_ecto3, :connect, :stop]` telemetry event as `error_reason`. One read-back caveat: `PRAGMA wal_autocheckpoint` queried through SQL always reports 0, because xqlite's WAL hook occupies SQLite's single slot and emulates the autocheckpoint. Read the effective value with `XqliteEcto3.with_xqlite(repo, &XqliteNIF.get_pragma(&1, "wal_autocheckpoint"))`.
...
- `diagnostics_budget_ms: 500` — the wall-clock allowance, in milliseconds, for each of the two error-path diagnoses: the unique-index-name lookup and the foreign-key replay. Both read the database back on a statement that has already failed while the caller waits, so the allowance is checked before every read and the diagnosis stops as soon as it is spent, degrading to what the adapter knew without it. The failed statement's own `:timeout` caps it: each spends the smaller of this allowance and what that deadline leaves once a 20 ms reserve is set aside, and is skipped only when the deadline is already inside the reserve. The foreign-key replay's reads additionally carry a cancel token fired at the allowance, so a `PRAGMA foreign_key_check` scan of a large database stops mid-scan instead of running to the end — except while it waits on another connection's lock, where the wait ends on `busy_timeout` instead. `0` turns both off. Anything but a non-negative integer is a structured connect error.
...
One wait `:timeout` does not bound at all: another connection's write lock. A blocked statement sits inside SQLite's busy handler, where the progress handler — the thing a cancel signals — never runs, so the call returns when `busy_timeout` expires (default 5000 ms), however small `:timeout` is, with a structured `%XqliteEcto3.Error{type: :database_busy_or_locked}`. That is any write, and on a rollback-journal database (`journal_mode: :delete` and friends, where a writer locks the whole file) a read as well. When lock waits must respect your deadline, set `busy_timeout` at or below it. Transaction start waits the same way: with the default `:immediate` mode, `BEGIN` takes the write lock inside the busy handler, so `Repo.transaction/2` can wait a full `busy_timeout` before failing with that error — and the failure keeps the connection rather than recycling it.

`:timeout` also decides how much of the error path the adapter pays for. When a statement fails, the two diagnoses below read the database back on the same connection and the same clock: each spends the smaller of `diagnostics_budget_ms` and what your deadline leaves once a 20 ms reserve is set aside, and only a deadline already inside that reserve skips one whole. The foreign-key replay also cancels a read that outlives that allowance — its `PRAGMA foreign_key_check` scans every table in the database, so on a large file it stops mid-scan and reports `{:unavailable, :operation_cancelled}` rather than the constraint's details. A skipped unique-index lookup reports the derived index name instead of the real one, so a changeset that declares a custom index name and no derived one raises `Ecto.ConstraintError` — see Real unique index names.
...
Four more caveats: the replay is a write, so under lock contention
it can wait up to one extra `busy_timeout` on top of the failed
statement's own wait, and neither the allowance nor the cancel token
shortens that wait; its `PRAGMA foreign_key_check` scans every table,
so on a large database the scan runs out of allowance and is
cancelled, reporting
`fk_diagnostics: {:unavailable, :operation_cancelled}` with no
violations attached; SQLite does not undo `last_insert_rowid()` on
...
- The TEXT-affinity mirror image of the same trap: a `:decimal` field over a `:string`/`TEXT` column silently stores SQLite's float-to-text rendering of the value — roughly one accepted decimal in ten comes back as a slightly different number, with no error anywhere. A `:decimal` field cannot be made exact by changing only the column. Three related facts about `:decimal`: the `precision:`/`scale:` migration options are carried into the DDL for documentation value only — SQLite ignores them and the exactness guard's limit is float64's, not the declared one; SQLite's `sum`/`avg` silently treat non-numeric stored values (a foreign writer's BLOB or text) as 0; and a non-numeric stored value under a `:decimal` field fails the load with Ecto's typed error naming the field and value rather than loading garbage, while numeric text of any width — a number a foreign writer put in the column, past the 34 significant digits and the 6144 exponent `Decimal` parses by default — loads as the number written. One value cannot survive at all: a `Decimal` negative zero binds as the integer `0`, and an INTEGER has no negative zero, so it loads back positive — the two compare equal, only the struct's `sign` field differs. The fix for exact arbitrary-precision digits is the `XqliteEcto3.Types.ExactDecimal` field type over that same `:string`/TEXT column: it binds the number as text with every digit kept, so nothing is rendered through a float — at the cost of textual, not numeric, ordering and comparison on the column.

- A `%Decimal{}` inside a `:map` field or an `{:array, :decimal}` field goes through the same precision guard as a bare one, and is written into the JSON as a number rather than a quoted string. A value the guard refuses is refused there too, with the same `XqliteEcto3.DecimalPrecisionError`. Under `{:array, :decimal}` each element loads back as a `Decimal`; under a plain `:map` field there is no per-value type to load through, so the numbers come back as Elixir integers and floats. The opt-in `XqliteEcto3.Types.Array` and `XqliteEcto3.Types.ExactDecimal` build their own JSON and text and are deliberately outside this guard.
...
- `alter table ... add` follows SQLite's own `ADD COLUMN` rule, which SQLite applies only once the table has rows: on an **empty** table every add succeeds, and on a **populated** one SQLite refuses a `null: false` column that carries no non-NULL constant default ("Cannot add a NOT NULL column with default value NULL"), so that migration can pass in dev/CI against a fresh database and fail in production. `timestamps()` inside an `alter table` block is the shape most people meet this through: it asks for `null: false` with no default. The other half of SQLite's rule — a default it does not read as a literal — is row-count dependent in the same way, so the adapter takes it out of SQLite's hands: an `add` whose `default:` is a `{:fragment, "..."}` is refused before the statement runs, empty table or not, with `XqliteEcto3.RebuildRefusedError` and reason `:non_constant_default_add`. Plain defaults — numbers, strings, booleans, and the adapter's JSON text defaults for maps/lists — are added either way; write a datetime constant in the form the adapter stores (`"2000-01-01 00:00:00"`, space separator, no trailing `Z`, per the datetime bullet above), or it will sort wrong against the values the adapter writes. Workaround when no constant fits: add the column nullable, backfill it with `execute/1`, then tighten it with `modify` under `support_alter_via_table_rebuild: true` (that path rewrites the table, so it reports `"NOT NULL constraint failed: ..."` instead if a row is still NULL) — and set the default there too if future rows need one. A `{:fragment, "..."}` default does work in that same rebuild block: put the `add` and a `modify` together and the fragment is evaluated for every row.

- `conflict_target: [:col]` renders as a bare column list, which SQLite matches only against a UNIQUE constraint or a plain unique index over exactly those columns. Against a **partial** index (`create index(:items, [:sku], unique: true, where: "active = 1")`) or an **expression** index, SQLite rejects the statement with `ON CONFLICT clause does not match any PRIMARY KEY or UNIQUE constraint`. Repeat the index's own definition instead — `conflict_target: {:unsafe_fragment, "(sku) WHERE active = 1"}` — with the columns in parentheses and the `WHERE` clause spelled exactly as the migration declared them. The fragment is passed through untouched, so build it from literals, never from user input.
...
- A raw `COMMIT`, `END` or `ROLLBACK` in an `Ecto.Adapters.SQL.Sandbox` test ends the sandbox's own transaction: everything the test wrote before that statement is committed to the database file for real, and the next test sees those rows. This holds for every Ecto adapter — the sandbox is a transaction like any other, and a statement in the test body can end it. Ecto has a diagnostic for exactly this case ("the sandbox transaction was already committed/rolled back"), and the adapter hands Ecto the transaction status that triggers it, but it is raised inside the sandbox's ownership process: it reaches you as a crash log, not as a failed assertion, and `Sandbox.checkin/1` still returns `:ok`. Nothing at check-in can undo the writes — undo them yourself, or keep transaction control out of the test body.
  ...
  That statement also ends every savepoint the sandbox opened, so a `Repo.transaction/2` around it cannot close the savepoint it opened. The adapter names that instead of letting SQLite refuse a savepoint it cannot find: with no transaction left, the close raises `%XqliteEcto3.Error{type: :savepoint_without_transaction}`; with a raw `BEGIN` after the raw `COMMIT`, so that a transaction is open again but the savepoints are not, it raises `type: :savepoint_counter_underflow`. Both drop that connection — its savepoint state is gone underneath the adapter — and the pool serves the next query on a replacement. One case stays SQLite's own `type: :sqlite_failure`: a raw `RELEASE` or `ROLLBACK TO` that removes savepoints while leaving the transaction open, which no side-effect-free read can detect.
...
The one exception is named CHECK constraints, where the name is only present in SQLite's error text and no PRAGMA exposes it. Parsing happens once, in Rust, at the NIF boundary — never in Elixir. An unnamed CHECK has no name to find there: SQLite writes the constraint's expression text in the same place, so that expression is what comes back, and no changeset declaration can match it (see Known limitations).
...
Every refusal on this path raises `XqliteEcto3.RebuildRefusedError`, before any destructive step. Beside the full explanation in its `message`, it carries `reason` — an atom naming the check that refused, such as `:rebuild_not_enabled`, `:unpreservable_construct`, `:dependents_exist`, `:primary_key_removed` or `:affinity_rewrite` — plus `table`, `construct`, `column`, `violations` (the `PRAGMA foreign_key_check` rows, when that is what refused) and `details` for the rest, so a migration tool can tell one refusal from another without reading prose.
