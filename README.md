...
- `alter table ... add` follows SQLite's own `ADD COLUMN` rule, which has two halves and one row-count switch: on an **empty** table every add succeeds, and on a **populated** one SQLite refuses both a non-constant `default:` fragment (`CURRENT_TIMESTAMP`, `(1+1)`) and a `null: false` column that carries no non-NULL constant default — "Cannot add a column with non-constant default" and "Cannot add a NOT NULL column with default value NULL". A migration can therefore pass in dev/CI against a fresh database and fail in production. `timestamps()` inside an `alter table` block is the shape most people meet this through: it asks for `null: false` with no default, and a `CURRENT_TIMESTAMP` default cannot rescue it because that is not a constant. Constant defaults — numbers, strings, booleans, and the adapter's JSON text defaults for maps/lists — are accepted either way; write a datetime constant in the form the adapter stores (`"2000-01-01 00:00:00"`, space separator, no trailing `Z`, per the datetime bullet above), or it will sort wrong against the values the adapter writes. Workaround when no constant fits: add the column nullable, backfill it with `execute/1`, then tighten it with `modify` under `support_alter_via_table_rebuild: true` (that path rewrites the table, so it reports `"NOT NULL constraint failed: ..."` instead if a row is still NULL) — and set the default there too if future rows need one.
