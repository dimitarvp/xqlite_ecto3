defmodule XqliteEcto3.Connection do
  @moduledoc """
  Implements `Ecto.Adapters.SQL.Connection` for SQLite via xqlite.

  The SQL generation layer (all/1, insert/7, execute_ddl/1, etc.) is adapted
  from ecto_sqlite3 v0.22.0 (MIT, by warmwaffles). The connection plumbing
  and error mapping are xqlite-native.
  """
  ...
  def execute(conn, %XqliteEcto3.Query{} = query, params, opts) do
    prepare_execute(conn, "", query.statement, params, opts)
  end

  def execute(conn, sql, params, opts) when is_binary(sql) or is_list(sql) do
    query = %XqliteEcto3.Query{statement: IO.iodata_to_binary(sql)}
  ...
  def stream(conn, sql, params, opts) do
    query = %XqliteEcto3.Query{statement: sql}
  ...
  def execute_ddl({_, %Index{only: true}}) do
    raise ArgumentError, "`only` is not supported with SQLite"
  end
  ...
  def execute_ddl({_, %Index{using: x}}) when not is_nil(x) do
    raise ArgumentError, "`using` is not supported with SQLite"
  end

  @impl true
  def execute_ddl({_, %Index{nulls_distinct: x}}) when not is_nil(x) do
    raise ArgumentError, "`nulls_distinct` is not supported with SQLite"
  end
        ...
        case elem(sources, idx) do
          {source, _, nil} ->
            raise Ecto.QueryError,
              query: query,
              message: """
              SQLite does not support selecting all fields from #{source} \
  ...
  defp update_op(command, _quoted_key, _value, _sources, query) do
    raise Ecto.QueryError,
      query: query,
      message: "Unknown update operation #{inspect(command)} for SQLite"
  end
  ...
  defp assert_valid_join(%JoinExpr{hints: hints}, query) when hints != [] do
    raise Ecto.QueryError,
      query: query,
      message: "join hints are not supported by SQLite"
  end
  ...
  defp expr({:ilike, _, [_, _]}, _sources, query) do
    raise Ecto.QueryError,
      query: query,
      message: "ilike is not supported by SQLite"
  end
  ...
  defp expr({:count, _, [{:&, _, [_]}]}, _sources, query) do
    raise Ecto.QueryError,
      query: query,
      message: "The argument to `count/1` must be a column in SQLite"
  end
  ...
  defp quote_table(prefix, _name) when is_atom(prefix) or is_binary(prefix) do
    raise ArgumentError, "SQLite does not support table prefixes"
  end
