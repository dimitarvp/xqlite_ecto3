defmodule XqliteEcto3.TelemetryTest do
  ...
  # Every test here asserts on an emitted event, so none is compiled into
  # the no-op build; that build's own coverage is the smoke file.
  if XqliteEcto3.Telemetry.enabled?() do
    ...
    describe "cursor lifecycle telemetry" do
      test "handle_declare / handle_fetch / handle_deallocate all fire", %{state: state} do
        ...
        assert_receive {:telemetry_event, [:xqlite_ecto3, :handle_deallocate, :stop], _,
                        dealloc_md}
    ...
    defp drain_events(acc \\ []) do
      receive do
        {:telemetry_event, name, _measurements, metadata} ->
          drain_events([{name, metadata} | acc])
      after
        0 -> Enum.reverse(acc)
      end
    end
