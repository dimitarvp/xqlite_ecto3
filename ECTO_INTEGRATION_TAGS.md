...
| `:microsecond_precision` | excluded (permanent, 4/5-justified) | SQLite's `strftime %f` is millisecond-precision; microsecond-exact datetime arithmetic rounds. Non-arithmetic µs round-trips via TEXT storage work fine (see types_test.exs). Not an adapter gap. Disclosure: the tag is over-broad by exactly one — `interval.exs:194` (`datetime_add with microsecond`) passes when re-enabled; keeping the tag over four location tuples is a recorded deliberate trade (see BACKLOG F-B2-8). |
